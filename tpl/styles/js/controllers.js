...
var ngApp;
ngApp = angular.module('app.controllers', ['pascalprecht.translate', 'ngCookies', 'ngSanitize'])
    ...
    //获取中国地址
    .controller('ChinaAddress', ['$scope', '$http', '$sce', function ($scope, $http, $sce) {
        $scope.address = [];
        $scope.ca_province = 0;
        $scope.ca_city = 0;
        $scope.ca_area = 0;
        ...
        $http.get('app/Access/address/province_json')
            .then(function (response) {
                var listHtml = '<option value="0">请选择省份</option>';
                ...
                $scope.address.list_province = $sce.trustAsHtml(listHtml);
            }, function (x) {
                $scope.addressMsg = '省份获取失败,请刷新';
            });

        $scope.address.list_city = $sce.trustAsHtml('<option value="">请先选择省份</option>');
        $scope.address.list_area = $sce.trustAsHtml('<option value="">请先选择城市</option>');


        //选择省份。城市
        $scope.addressChangeP = function () {
            if ($scope.ca_province == '') {
                $scope.addressMsg = '请先选择省份';
                return false;
            }

            $http.get('app/Access/address/city_json/' + $scope.ca_province)
                .then(function (response) {
                    var listHtml = '<option value="0">请选择城市</option>';
                    if (response.data.status == 'success') {
                        angular.forEach(response.data.json, function (data) {
                            listHtml += '<option value="' + data['id'] + '">' + data['name'] + '</option>';
                        });
                        $scope.address.list_city = $sce.trustAsHtml(listHtml);
                    } else {
                        $scope.addressMsg = '城市获取失败,请重新选择';
                    }
                }, function (x) {
                    $scope.addressMsg = '城市获取失败,请重新选择';
                });

            $('#ca_box_city').removeClass('hidden');
        };
        //城市
        $scope.addressChangeC = function () {
            if ($scope.h_city == '') {
                $scope.addressMsg = '请先选择城市';
                return false;
            }

            $http.get('app/Access/address/area_json/' + $scope.ca_city)
                .then(function (response) {
                    var listHtml = '<option value="0">请选择县/区</option>';
                    if (response.data.status == 'success') {
                        angular.forEach(response.data.json, function (data) {
                            listHtml += '<option value="' + data['id'] + '">' + data['name'] + '</option>';
                        });
                        $scope.address.list_area = $sce.trustAsHtml(listHtml);
                    } else {
                        $scope.addressMsg = '县/区获取失败,请重新选择';
                    }
                }, function (x) {
                    $scope.addressMsg = '县/区获取失败,请重新选择';
                });

            $('#ca_box_area').removeClass('hidden');
        };
    }])
    ...
    .controller('SigninFormController', ['$scope', '$http', '$state', function ($scope, $http, $state) {
        ...
        $scope.login = function () {
            $scope.authError = null;
            // Try to login
            $http.post('app/Access/sign/sign_in', {email: $scope.user.email, password: $scope.user.password})
                .then(function (response) {
                    if (response.data.status == 'error') {
                        ...
                        //$state.go('app/Index/index', {}, {reload: true});
                    }
                }, function (x) {
